<div class="row mb-2">
    <div class="col-md-4">
        <div class="mb-3">
            <x-form-input name="name" value="{{ $branch->name ?? '' }}" label="Branch Name"
                placeholder="Enter Branch Name" required="true" />
        </div>
    </div>
    <div class="col-md-4">
        <div class="mb-3">
            <x-form-input name="short_name" value="{{ $branch->short_name ?? '' }}" label="Branch Short Name"
                placeholder="Enter Short Name" required="true" />
        </div>
    </div>
    <div class="col-md-4">
        <div class="mb-3">
            <x-form-input name="email" value="{{ $branch->email ?? '' }}" label="Email" placeholder="Enter Email"
                required="true" />
        </div>
    </div>
    <div class="col-md-4">
        <div class="mb-3">
            <x-form-input name="contact" type="number" value="{{ $branch->contact ?? '' }}" label="Contact"
                placeholder="Enter Contact" required="true" />
        </div>
    </div>
    <div class="col-md-4">
        <div class="mb-3">
            <x-form-input name="location" value="{{ $branch->location ?? '' }}" label="Location"
    ...
    <div class="col-md-4">
        ...
        <select name="status" class="form-control">
            <option value="Active" @isset($branch) @selected($branch->status == 'Active') @endisset>Active
            </option>
            <option value="Inactive" @isset($branch) @selected($branch->status == 'Inactive') @endisset>Inactive
            </option>
        </select>
    </div>
    <div class="col-md-4">
